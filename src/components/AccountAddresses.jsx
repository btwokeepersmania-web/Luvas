                  ...
                  <div className="space-y-2">
                    ...
                    {isTouchDevice ? (
                      <>
                        ...
                        >
                          <span>{formData.country || t('account.addresses.selectCountry')}</span>
                          <ChevronDown className="ml-2 h-4 w-4" />
                        </Button>
                        <Dialog open={isCountryModalOpen} onOpenChange={setIsCountryModalOpen}>
                          <DialogContent className="bg-gray-900 border-yellow-500/20 text-white max-w-lg w-[90vw] p-0">
                            <DialogHeader className="p-4 border-b border-yellow-500/20">
                              <DialogTitle className="text-yellow-400 text-lg">
                                {t('account.addresses.selectCountry')}
                              </DialogTitle>
                            </DialogHeader>
                            <div className="p-4 space-y-4">
                              <Input
                                autoFocus
                                placeholder={tWithFallback(t, 'account.addresses.searchCountry', 'Search country')}
                                value={countrySearch}
                                onChange={(event) => setCountrySearch(event.target.value)}
                                className="bg-gray-800 border-gray-700"
                              />
                              <div className="max-h-64 overflow-y-auto border border-gray-800 rounded-md divide-y divide-gray-800">
                                {filteredCountryOptions.map((option) => (
                                  <button
                                    type="button"
                                    key={option.code}
                                    onClick={() => handleSelectCountry(option.code)}
                                    className="w-full text-left px-4 py-3 hover:bg-gray-800 focus:outline-none focus:bg-gray-800"
                                  >
                                ...
                                {filteredCountryOptions.length === 0 && (
                                  <div className="px-4 py-3 text-sm text-gray-400 text-center">
                                    {tWithFallback(t, 'account.addresses.noResults', 'No countries found')}
                                  </div>
                                )}
                              </div>
                              <Button
                                ...
                                className="w-full border border-gray-700 text-gray-300 hover:bg-gray-800"
                              >
                                {tWithFallback(t, 'common.cancel', 'Cancel')}
                              </Button>
                            </div>
                          </DialogContent>
                        </Dialog>
